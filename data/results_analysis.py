...
def analyze(args):
    ...
    # df["l1_pre_suff_diff"] = df[f"global_filled_prefix_l1{lsuff}"] - \
    #     df[f"global_filled_suffix_l1{lsuff}"]
    # df = df.sort_values(by="l1_pre_suff_diff", ascending=False)
    df = df.sort_values(by=f"global_filled_suffix_l1{lsuff}", ascending=False)

                ...
                if self.phonotactic_reduction:
                    if RG.random() < self.model.phonotactic_reduction_prob:
                        prefix = misc.reduce_phonotactics(
                            "prefix", prefix, form, self.model.clts, self.model.cv_pattern_cache, drop_boundary_phoneme=self.model.phonotactic_reduction_drop_boundary_phoneme)
            else:
                ...
                if self.phonotactic_reduction:
                    if RG.random() < self.model.phonotactic_reduction_prob:
                        suffix = misc.reduce_phonotactics(
                            "suffix", suffix, form, self.model.clts, self.model.cv_pattern_cache, drop_boundary_phoneme=self.model.phonotactic_reduction_drop_boundary_phoneme)
            else:
                # Just skip this whole interaction. Only listen for this concept until it gets filled with at least one form.
                return
